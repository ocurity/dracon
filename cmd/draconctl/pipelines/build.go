...
import (
	...
	"github.com/ocurity/dracon/pkg/components/generator/backends"
...
func buildPipeline(cmd *cobra.Command, args []string) error {
	...
	for _, pathOrURI := range kustomization.Components {
		...
		if err = components.ValidateTask(&newTask); err != nil {
			return fmt.Errorf("%s: invalid task found: %w", newTask.Metadata.Name, err)
		}

		newTask.Metadata.Namespace = kustomization.Namespace
		taskList = append(taskList, newTask)
	}

	k8sBackend, err := backends.NewTektonBackend(&basePipeline, taskList, kustomization.NamePrefix, kustomization.NameSuffix)
	if err != nil {
