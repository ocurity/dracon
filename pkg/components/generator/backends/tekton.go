...
func fixTaskPrefixSuffix(task tektonV1Beta1.Task, prefix, suffix string) {
	...
	for _, param := range task.Spec.Parameters {
		...
		paramNameChunks := strings.Split(param.Name, task.Metadata.Name)
		param.Name = prefix + task.Metadata.Name + suffix + paramNameChunks[1]
	...
	task.Metadata.Name = prefix + task.Metadata.Name + suffix
}
